const UPSTREAM = new URL("https://robertzaufall.github.io/openclaw-repos/");
const PREFIX = "/openclaw/";

export default {
  async fetch(request) {
    const source = new URL(request.url);
    if (source.pathname !== "/openclaw" && !source.pathname.startsWith(PREFIX)) {
      return new Response("Not found", { status: 404 });
    }
    if (!["GET", "HEAD"].includes(request.method)) {
      return new Response("Method not allowed", { status: 405, headers: { Allow: "GET, HEAD" } });
    }
    if (source.pathname === "/openclaw") {
    ...
    headers.set("x-rob-proxy", "openclaw");
